...
use witnessd_engine::vdf::params::calibrate;
use witnessd_engine::{derive_hmac_key, SecureStore};
use zeroize::Zeroizing;

use witnessd_engine::config::WitnessdConfig;

use crate::util::{ensure_dirs, open_secure_store, witnessd_dir};
...
pub(crate) fn cmd_status() -> Result<()> {
    ...
    if db_path.exists() {
        let hmac_key =
            if let Ok(Some(key)) = witnessd_engine::identity::SecureStorage::load_hmac_key() {
                Some(key.to_vec())
            } else {
