...
//! Witnessd CLI — cryptographic authorship witnessing.
...
async fn run() -> Result<()> {
    ...
    if should_auto_start {
        if let Ok(dir) = util::witnessd_dir() {
            if let Ok(config) = witnessd_engine::config::WitnessdConfig::load_or_default(&dir) {
                if config.sentinel.auto_start {
                    let daemon_manager =
                        witnessd_engine::DaemonManager::new(config.data_dir.clone());
