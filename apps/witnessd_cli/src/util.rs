...
//! Shared utility functions for the witnessd CLI.
...
use witnessd_engine::vdf::params::Parameters as VdfParameters;
use witnessd_engine::{derive_hmac_key, SecureStore};
use zeroize::Zeroize;

pub fn witnessd_dir() -> Result<PathBuf> {
...
pub fn ensure_dirs() -> Result<WitnessdConfig> {
    ...
    for d in &dirs {
        ...
        // The .witnessd directory contains keys, events, and other sensitive data.
        #[cfg(unix)]
        {
...
pub fn load_vdf_params(config: &WitnessdConfig) -> VdfParameters {
...
pub fn get_device_id() -> Result<[u8; 16]> {
    ...
    let pub_key = fs::read(&key_path).map_err(|e| {
        anyhow::anyhow!("Cannot read signing_key.pub (run `witnessd init` first): {e}")
    })?;
