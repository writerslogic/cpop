...
use cpoe::identity::SecureStorage;
use cpoe::keyhierarchy::{derive_master_identity, SoftwarePUF};
use cpoe::tpm;
...
pub(crate) fn cmd_init() -> Result<()> {
    ...
    {
        println!("CPoE is already initialized.");
        println!("  Data directory: {}", dir.display());
        println!();
        println!("To start fresh, run: cpoe identity --recover");
        return Ok(());
    }
    ...
    if config.vdf.iterations_per_second == 0 {
        ...
        let calibrated = cpoe::vdf::params::calibrate(std::time::Duration::from_secs(2))
