...
/// Output from a cpop CLI invocation.
...
impl TempEnv {
    ...
    /// Create a test environment with CPOP initialized (identity + keys).
    ...
    /// Run cpop expecting success; panics with details on failure.
    ...
    /// Run cpop expecting failure; panics if it succeeds.
    ...
    /// Initialize cpop and create a file with N commits already made.
...
/// Run cpop in a specific directory with environment isolation.
fn run_cpop_in(
    dir: &Path,
    bin: &str,
    args: &[&str],
