...
This specification defines the high-integrity hardening measures implemented in `wld` to protect cryptographic material and evidence capture logic against a **white-box adversary** (a local user with root privileges and debugging tools).
...
To prevent the operating system from swapping sensitive keys to disk (where they could be recovered via forensic analysis of the swap file), `wld` utilizes the `mlock` system call.
...
To prevent real-time manipulation of the forensic capture loop or the VDF computation, `wld` implements platform-specific anti-analysis measures.
