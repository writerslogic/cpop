...
| `public_key` | bytes[32] | No | Their WritersLogic public key (if they have one) |
...
func ComputeSigningPayload(d *Declaration) ([]byte, error) {
    ...
    h.Write([]byte("wld-declaration-v1"))
...
1. **Domain Separation**: Prefix with `"wld-declaration-v1"` to prevent cross-protocol attacks
