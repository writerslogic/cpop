...
use crate::wld_jitter_bridge::doc_tracker::DocumentTracker;
use crate::wld_jitter_bridge::types::{
    EntropyQuality, HybridEvidence, HybridSample, HybridSessionData,
};
use crate::wld_jitter_bridge::zone_engine::ZoneTrackingEngine;
...
impl HybridJitterSession {
    pub fn new(
        ...
        let wld_jitter_session = PhysSession::new(secret);
    ...
    pub fn record_keystroke(&mut self, keycode: u16) -> Result<(u32, bool), String> {
        ...
        let jitter = self
            .wld_jitter_session
            .sample(&input)
            .map_err(|e| format!("wld_jitter sample failed: {e}"))?;
    ...
    pub fn export(&self) -> HybridEvidence {
        ...
        HybridEvidence {
            ...
            wld_jitter_evidence: self.wld_jitter_session.export_json().ok(),
        }
    }
    ...
    pub fn load(path: impl AsRef<Path>, key_material: Option<[u8; 32]>) -> Result<Self, String> {
        ...
        Ok(Self {
            wld_jitter_session: PhysSession::new(secret),
...
impl HybridEvidence {
    pub fn verify(&self) -> Result<(), String> {
        ...
        if let Some(ref wld_jitter_json) = self.wld_jitter_evidence {
            self.verify_wld_jitter_evidence(wld_jitter_json)?;
        }

        Ok(())
    }

    fn verify_wld_jitter_evidence(&self, json: &str) -> Result<(), String> {
        let chain: PhysEvidenceChain = serde_json::from_str(json)
            .map_err(|e| format!("wld_jitter evidence parse error: {e}"))?;

        if !chain.validate_sequences() {
            return Err("wld_jitter evidence: sequence numbers not monotonic".to_string());
        }

        if !chain.validate_timestamps() {
            return Err("wld_jitter evidence: timestamps not monotonic".to_string());
        }
