...
fn decode_evidence_for_c2pa(
    data: &[u8],
) -> std::result::Result<wld_protocol::rfc::EvidencePacket, String> {
    ...
    for (i, cp) in packet.checkpoints.iter().enumerate() {
        ...
        checkpoints.push(wld_protocol::rfc::Checkpoint {
            sequence: cp.ordinal,
            checkpoint_id,
            timestamp: cp.timestamp.timestamp_millis() as u64,
            content_hash: wld_protocol::rfc::HashValue {
                algorithm: wld_protocol::rfc::HashAlgorithm::Sha256,
                digest: hex::decode(&cp.content_hash).map_err(|e| ctx("content_hash", &e))?,
            },
            char_count: cp.content_size,
            prev_hash: wld_protocol::rfc::HashValue {
                algorithm: wld_protocol::rfc::HashAlgorithm::Sha256,
                digest: hex::decode(&cp.previous_hash).map_err(|e| ctx("previous_hash", &e))?,
            },
            checkpoint_hash: wld_protocol::rfc::HashValue {
                algorithm: wld_protocol::rfc::HashAlgorithm::Sha256,
                digest: hash_bytes,
            },
            jitter_hash: None,
