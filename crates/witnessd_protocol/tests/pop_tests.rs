...
fn test_pop_full_roundtrip() {
    ...
    builder.add_checkpoint(b"Checkpoint 1", 12).expect("Add checkpoint failed");
    builder.add_checkpoint(b"Checkpoint 2", 12).expect("Add checkpoint failed");

    let signed_evidence = builder.finalize().expect("Finalize failed");

    let verifier = PoPVerifier::new(verifying_key);
    let result = verifier.verify(&signed_evidence).expect("Verification failed");
...
fn test_pop_playback_attack_detection() {
    use witnessd_protocol::rfc::{EvidencePacket, Checkpoint, AttestationTier};
    ...
    for i in 0..4 {
        ...
        let checkpoint_hash = witnessd_protocol::crypto::compute_causality_lock(
            &packet_id, &last_hash.digest, &content_hash.digest
        ).unwrap();
    ...
    let signed = sign_evidence_cose(&encoded, &signing_key).unwrap();

    let verifier = PoPVerifier::new(verifying_key);
    let result = verifier.verify(&signed);

    assert!(result.is_err(), "Verifier should catch uniform timing (Adversarial Collapse)");
    if let Err(witnessd_protocol::error::Error::Validation(msg)) = result {
        assert!(msg.contains("Adversarial collapse"));
    } else {
        panic!("Expected Validation Error with 'Adversarial collapse', got {:?}", result);
    }
}
...
fn test_identity_csr_generation() {
    ...
    let csr_der = id_manager.generate_csr("CN=PopDevice,O=WritersLogic,C=US")
