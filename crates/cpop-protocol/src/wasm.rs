...
pub fn verify_cpop_evidence(evidence_bytes: &[u8], public_key_bytes: &[u8]) -> VerificationResult {
    ...
    match verifier.verify(evidence_bytes) {
        ...
        Err(e) => {
            // BRITTLE: verdict classification relies on substring matching in error messages.
            // If error messages in evidence.rs change, these branches may silently stop matching.
            let verdict = match &e {
