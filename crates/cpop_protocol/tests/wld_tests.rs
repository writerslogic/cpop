...
use cpop_protocol::evidence::{PoPBuilder, PoPVerifier};
...
fn test_pop_playback_attack_detection() {
    ...
    let signer: Box<dyn cpop_protocol::crypto::PoPSigner> = Box::new(signing_key);
    ...
    if let Err(cpop_protocol::error::Error::Validation(msg)) = result {
