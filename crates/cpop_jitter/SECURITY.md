...
This document outlines the security policy for the `cpop_jitter` crate, including vulnerability reporting, threat model, security considerations, and secure usage guidelines.
...
`cpop_jitter` is designed to provide proof-of-process through timing jitter. It is **NOT**:
...
```rust
use cpop_jitter::{HybridEngine, Session};
...
curl -LO https://github.com/writerslogic/writerslogic/releases/download/v0.2.0/cpop_jitter-0.2.0.crate
curl -LO https://github.com/writerslogic/writerslogic/releases/download/v0.2.0/multiple.intoto.jsonl

# Verify
slsa-verifier verify-artifact cpop_jitter-0.2.0.crate \
