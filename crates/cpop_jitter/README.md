<p align="center">
  <img src="assets/logo.png" alt="cpop_jitter" width="200">
</p>

<h1 align="center">cpop_jitter</h1>
...
<p align="center">
  <a href="https://crates.io/crates/cpop_jitter"><img src="https://img.shields.io/crates/v/cpop_jitter.svg" alt="Crates.io"></a>
  <a href="https://docs.rs/cpop_jitter"><img src="https://docs.rs/cpop_jitter/badge.svg" alt="Documentation"></a>
...
```toml
# Enable all features
cpop_jitter = { version = "0.2", features = ["hardware", "rand"] }

# no_std (embedded/WASM compatible)
cpop_jitter = { version = "0.2", default-features = false }
```
...
```rust
use cpop_jitter::{HybridEngine, Evidence, Error};
...
```rust
use cpop_jitter::{PureJitter, JitterEngine};
...
```rust
use cpop_jitter::{PhysJitter, EntropySource, JitterEngine, Error};
...
```rust
use cpop_jitter::{Session, HumanModel, Jitter};
...
```rust
use cpop_jitter::{Session, ValidationResult, AnomalyKind};
...
```rust
use cpop_jitter::{EvidenceChain, Evidence};
...
```rust
use cpop_jitter::{EvidenceChain, Evidence, PureJitter, JitterEngine};
...
For complete API documentation, see [docs.rs/cpop_jitter](https://docs.rs/cpop_jitter).
...
```rust
use cpop_jitter::{Session, HybridEngine, PhysJitter, PureJitter};
...
Plain timestamps can be easily forged. `cpop_jitter` creates evidence that's cryptographically bound to both the secret and the hardware.
...
| Feature | cpop_jitter | Timestamp logging | Behavioral biometrics |
|---------|------------|-------------------|----------------------|
...
- [cpop_engine](../cpop_engine/) — Core cryptographic engine
- [cpop_protocol](../cpop_protocol/) — Wire protocol and CBOR encoding
