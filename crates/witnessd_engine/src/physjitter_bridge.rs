...
//! Bridge module integrating witnessd_jitter crate with witnessd's zone-aware typing profiles.
//!
//! This module provides [`HybridJitterSession`] which combines:
//! - witnessd_jitter's hardware entropy (PhysJitter) with automatic fallback (HybridEngine)
...
//!
//! ```rust,ignore
//! use witnessd_engine::witnessd_jitter_bridge::HybridJitterSession;
//!
//! let mut session = HybridJitterSession::new("/path/to/document.txt", None)?;
//!
//! // Record keystrokes - automatically samples witnessd_jitter + zones
...
/// Hybrid jitter session combining witnessd_jitter with witnessd's zone tracking.
///
/// This session type uses witnessd_jitter's hardware entropy when available,
...
pub struct HybridJitterSession {
    /// Underlying witnessd_jitter session.
    witnessd_jitter_session: PhysSession,
...
impl HybridJitterSession {
    ...
    /// * `key_material` - Optional master key material for session secret derivation.
    ...
    pub fn new(
        ...
        // Use provided key material or generate random
    ...
    pub fn record_keystroke(&mut self, keycode: u16) -> Result<(u32, bool), String> {
        ...
        // Build input for witnessd_jitter (combines keystroke context)
    ...
    ///
    /// Note: Providing the original `key_material` allows resuming the cryptographic chain.
    /// If `key_material` is None, a random key is used (suitable for read-only viewing).
    pub fn load(path: impl AsRef<Path>, key_material: Option<[u8; 32]>) -> Result<Self, String> {
        ...
        // Use provided key material or generate random (read-only mode)
        let material = if let Some(k) = key_material {
            k
        } else {
            let mut k = [0u8; 32];
...
struct HybridSessionData {
    ...
    last_jitter: u32,
    zone_engine: ZoneTrackingEngine,
    witnessd_jitter_evidence: Option<String>,
}

/// Extended evidence format including witnessd_jitter metrics.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HybridEvidence {
    ...
    /// Raw witnessd_jitter evidence (JSON string).
    pub witnessd_jitter_evidence: Option<String>,
}

impl HybridEvidence {
    /// Verify the evidence chain including embedded witnessd_jitter evidence.
    ///
    /// This verifies:
    ...
    pub fn verify(&self) -> Result<(), String> {
        ...
        // Verify embedded witnessd_jitter evidence chain (if present)
    ...
    /// Verify the embedded witnessd_jitter evidence chain.
